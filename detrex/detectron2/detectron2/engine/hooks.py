...
class BestCheckpointer(HookBase):
    ...
    def __init__(
        ...
        """
        Args:
            eval_period (int): the period `EvalHook` is set to detectron2_run.
...
class TorchProfiler(HookBase):
    ...
    The above example will detectron2_run the profiler for iteration 10~20 and dump
...
class EvalHook(HookBase):
    ...
    def __init__(self, eval_period, eval_function, eval_after_train=True):
        """
        Args:
            eval_period (int): the period to detectron2_run `eval_function`. Set to 0 to
...
class PreciseBN(HookBase):
    ...
    def __init__(self, period, model, data_loader, num_iter):
        """
        Args:
            period (int): the period this hook is detectron2_run, or 0 to not detectron2_run during training.
                The hook will always detectron2_run in the end of training.
            model (nn.Module): a module whose all BN layers in training mode will be
                updated by precise BN.
                Note that user is responsible for ensuring the BN layers to be
                updated are in training mode when this hook is triggered.
            data_loader (iterable): it will produce data to be detectron2_run by `model(data)`.
